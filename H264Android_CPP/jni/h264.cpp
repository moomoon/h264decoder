...
static uint8_t *decode_nal(H264Context *h, uint8_t *src, int *dst_length, int *consumed, int length)
{
    ...
    h->rbsp_buffer= (unsigned char*)av_fast_realloc(h->rbsp_buffer, &h->rbsp_buffer_size, length);
    dst= h->rbsp_buffer;
...
static int alloc_tables(H264Context *h)
{
	...
	h->non_zero_count    =(unsigned char (*)[16])av_mallocz( big_mb_num * 16 * sizeof(uint8_t));
	h->slice_table_base   =(unsigned char *)av_mallocz( big_mb_num * sizeof(uint8_t));
	h->top_border        =(unsigned char (*)[32])av_mallocz( s->mb_width * (16+8+8) * sizeof(uint8_t));

	if( h->pps.cabac )
	{
		h->chroma_pred_mode_table =(unsigned char *)av_mallocz( big_mb_num * sizeof(uint8_t));
		h->cbp_table =(unsigned short *)av_mallocz( big_mb_num * sizeof(uint16_t));
		h->mvd_table[0] =(short (*)[2])av_mallocz( 32*big_mb_num * sizeof(uint16_t));
		h->mvd_table[1] =(short (*)[2])av_mallocz( 32*big_mb_num * sizeof(uint16_t));
	}

	memset(h->slice_table_base, -1, big_mb_num  * sizeof(uint8_t));
	h->slice_table= h->slice_table_base + s->mb_stride + 1;

	h->mb2b_xy   =(unsigned short *)av_mallocz( big_mb_num * sizeof(uint16_t));
	h->mb2b8_xy  =(unsigned short *)av_mallocz( big_mb_num * sizeof(uint16_t));
...
static int decode_ref_pic_marking(H264Context *h)
{
	...
	else
	{
		...
		{ 
			...
			{
                MMCOOpcode opcode= (enum MMCOOpcode)get_ue_golomb(&s->gb);
